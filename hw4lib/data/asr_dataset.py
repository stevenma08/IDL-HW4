...
class ASRDataset(Dataset):
    def __init__(
        ...
        for i in tqdm(range(self.length)):
            ...
            if feat.shape[1] == self.config['num_feats']: # If (T, F), transpose to (F, T)
                feat = feat.T
            ...
            if self.partition != "test-clean":
                ...
                transcript = str(np.load(text_path)[0])
                ...
                self.transcripts_shifted.append(torch.tensor([self.sos_token] + tokenized, dtype=torch.long))
                self.transcripts_golden.append(torch.tensor(tokenized + [self.eos_token], dtype=torch.long))
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        feat_lengths = torch.tensor([x.size(0) for x in batch_feats], dtype=torch.long)
        ...
        padded_feats = pad_sequence(batch_feats, batch_first=True, padding_value=0)
        ...
        if self.partition != "test-clean":
            ...
            transcript_lengths = torch.tensor([x.size(0) for x in batch_shifted], dtype=torch.long)
