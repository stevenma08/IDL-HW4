...
class ASRTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        pad_idx = getattr(tokenizer, "pad_id", tokenizer.eos_id)   # fallback if no PAD
        ls_val  = config.get("loss", {}).get("label_smoothing", 0.0)
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            if not isinstance(transcript_lengths, torch.Tensor):
                transcript_lengths = torch.tensor(transcript_lengths, device=feats.device, dtype=torch.long)
    ...
    def recognize(self, dataloader, recognition_config: Optional[Dict[str, Any]] = None, config_name: Optional[str] = None, max_length: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        if recognition_config is None:
            recognition_config = defaults.copy()
            config_name = 'greedy'
        else:
            for k, v in defaults.items():
                recognition_config.setdefault(k, v)
